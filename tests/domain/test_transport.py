...
from robot.domain.transport import classify_status, raise_for_status, warm_endpoints
from robot.domain.types import Endpoint
...
async def test_warm_endpoints_only_gets_endpoints_marked_warm() -> None:
    hit: list[str] = []
    ...
    endpoints = (
        Endpoint(name="home", url="https://home.test", warm=True),
        Endpoint(name="api", url="https://api.test", warm=False),
    )
    async with _client(handler) as client:
        await warm_endpoints(client, endpoints)
    assert hit == ["https://home.test"]
...
async def test_warm_endpoints_raises_on_the_first_fault_without_checking_the_rest() -> (
    ...
    endpoints = (
        Endpoint(name="first", url="https://first.test", warm=True),
        Endpoint(name="second", url="https://second.test", warm=True),
    )
    async with _client(handler) as client:
