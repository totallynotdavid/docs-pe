...
from tests.conftest import fake_site as make_site
...
def test_a_missing_export_file_imports_nothing(
    ...
    site = make_site("osiptel", "carrier", "lines", "total_lines")
...
def test_a_header_mismatch_imports_nothing(
    store: OutcomeStore,
    tmp_path: Path,
) -> None:
    site = make_site("osiptel", "carrier", "lines", "total_lines")
    ...
    assert imported == 0
    assert list(store.success_rows(site.name)) == []
...
def test_a_row_with_the_wrong_column_count_is_skipped(
    ...
    assert (site.name, "20100000001") not in store.done_pairs()


def test_an_invalid_doc_row_is_skipped(
    store: OutcomeStore,
    tmp_path: Path,
) -> None:
    site = make_site("osiptel", "carrier", "lines", "total_lines")
...
def test_rows_for_the_same_doc_are_grouped_into_one_import(
    ...
    assert (site.name, "20100000001") in store.done_pairs()
