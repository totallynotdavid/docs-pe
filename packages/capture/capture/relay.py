...
class RelayState:
    ...
    def record(self, payload: object) -> None:
        current_ruc = self.current_ruc

        if current_ruc is None:
            msg = "the relay has no pending RUC"
            raise ValueError(msg)

        if not isinstance(payload, dict) or payload.get("ruc") != current_ruc:
        ...
        try:
            result = self.site.parse(payload, expected_ruc=current_ruc)
            ...
            self.rejected += 1
            print(f"REJECTED {current_ruc}", flush=True)
            ...
            self.failed += 1
            print(f"FAILED {current_ruc}: {exc}", flush=True)
        else:
            previous = self.store.latest(self.site.name, current_ruc)
            ...
            print(
                (
                    f"OK {current_ruc} {_summary(result.columns)} "
                    f"({result.elapsed_ms} ms){marker}"
                ),
                flush=True,
            )
            self.succeeded += 1
