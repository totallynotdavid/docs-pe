...
def test_zero_debt_is_a_success_when_has_error_is_false() -> None:
    ...
    result = parse_lookup_result(_payload(debt=debt), expected_ruc=RUC)
...
def test_response_for_another_ruc_is_rejected() -> None:
    with pytest.raises(BrowserError, match="another RUC"):
        parse_lookup_result(_payload(ruc="20100000001"), expected_ruc=RUC)
...
def test_invalid_debt_total_is_rejected(total: str) -> None:
    ...
    with pytest.raises(BrowserError, match="total is invalid"):
        parse_lookup_result(_payload(debt=debt), expected_ruc=RUC)
