...
the source state-database path. Future entries should record those details with
the input count, selected sites, provider mix, retryable count, terminal-failure
count, and measurement date. Without that provenance, compare jobs cautiously.

The state database is authoritative. CSVs are projections and can omit a
successful document when the site returned zero rows.

## Reconcile by document outcome

For OSIPTEL, an empty result is a successful result:

```text
successful documents with lines
+ successful documents with zero lines
+ terminal failures
= accepted input documents
```

For SUNAT identity lookups, every accepted document should have one outcome:

```text
successful documents + not-found documents + failed documents
= accepted input documents
```

Separate failed documents into retryable and terminal before declaring a job
complete. A failed row in `out.<site>.errors.csv` is not necessarily terminal.

For `sunat_reps`, a successful empty result is valid because an entity may have
no legal representatives. More than one output row for an input RUC is also
valid. The current rules are in [the SUNAT site note](../sites/sunat.md).
...
The following carrier aggregate predates the inclusion of five district jobs
in the broader breakdown:
...
The rows preserve the historical table. The table does not identify which
errors remained retryable at the time it was recorded. Use the current state
database and [troubleshooting](../operations/troubleshooting.md) for a live
job.
...
These measurements are dated 2026-08-01. They should not be compared with a
new run without recording the same provider, lane, session-budget, and site
conditions.
...
GeoNode recorded 9.5 seconds per lookup at 15 lanes and 7.9 seconds at 60
lanes in those runs. DataImpulse stayed stable at 20 lanes in a 600-document
OSIPTEL probe.

## Historical transfer and cost observations

The origin transferred roughly 16 KB per OSIPTEL lookup at a six-second
session budget of one, and roughly 26 KB per SUNAT lookup with a session budget
of 50. A 235,000-document SUNAT job transferred about 6 GB. Providers bill a
proxied session rather than an individual lookup, so the session budget changes
the cost model.
